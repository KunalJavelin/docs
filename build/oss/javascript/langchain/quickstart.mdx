...
<Tip>
You can use [any supported model](/oss/javascript/integrations/providers/overview) by changing the model name and setting up the appropriate API key. Trace what's happening inside your agent with [LangSmith](https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=oss-langchain-quickstart)—follow the [tracing quickstart](/langsmith/trace-with-langchain) to get set up.
</Tip>
