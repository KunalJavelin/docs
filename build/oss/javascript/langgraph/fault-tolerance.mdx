...
Configure retries, timeouts, and error handlers per node via [`addNode`](https://reference.langchain.com/javascript/langchain-langgraph/index/StateGraph#member-addNode-3) options. `setNodeDefaults` is not yet available in the JavaScript/TypeScript SDK.
...
<Note>
Per-node timeouts and node-level error handlers require a recent `@langchain/langgraph` release. `setNodeDefaults` is not yet available in the JavaScript/TypeScript SDK.
</Note>
...
Pass `retryPolicy` to [`addNode`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.Graph.html#addnode):
...
By default, the retry policy retries on **any** exception except the following:

- `TypeError`
- `SyntaxError`
- `ReferenceError`

[`NodeTimeoutError`](https://reference.langchain.com/javascript/langchain-langgraph/index/NodeTimeoutError) is retryable by default.
...
const fanOut = (state: typeof State.State) =>
  state.items.map(
    (item) => new Send("processItem", { item }, { idleTimeout: 15_000 })
  );
```
...
The `timeout` and `retryPolicy` options are available on `task` and `entrypoint` in the functional API:
...
- **`setNodeDefaults` is Python-only**: configure retries, timeouts, and error handlers per node via `addNode` options instead.
