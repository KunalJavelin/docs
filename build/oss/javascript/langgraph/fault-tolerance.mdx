...
Configure retries, timeouts, and error handlers per node via [`addNode`](https://reference.langchain.com/javascript/langchain-langgraph/index/StateGraph#member-addNode-3) options. For graph-wide **`retryPolicy`** and **`cachePolicy`** defaults, see [`setNodeDefaults`](#graph-defaults).
...
<Note>
Per-node timeouts and node-level error handlers require a recent `@langchain/langgraph` release.
</Note>
...
Pass `retryPolicy` to [`addNode`](https://reference.langchain.com/javascript/langchain-langgraph/index/StateGraph#member-addNode-3):
...
By default, when `retryOn` is omitted, the built-in handler retries on **any** thrown error except:

- Abort/cancel errors (`AbortError`, or messages starting with `"Cancel"` or `"AbortError"`)
- `GraphValueError`
- Connection aborted errors (`ECONNABORTED`)
- Common HTTP 4xx responses (400, 401, 402, 403, 404, 405, 406, 407, 409) from `fetch`, Axios, or similar clients
- OpenAI `insufficient_quota` errors

[`NodeTimeoutError`](https://reference.langchain.com/javascript/langchain-langgraph/index/NodeTimeoutError) is retryable by default. There is no exported equivalent of Python's `default_retry_on`—provide your own `retryOn` callback to extend or replace this behavior.
...
| `jitter` | `boolean` | `false` | Add random jitter to the interval. |
| `retryOn` | `(error: unknown) => boolean` | built-in handler | Callable returning `true` for retryable exceptions. |
...
The `timeout` parameter on [`add_node`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html#addNode) / [`addNode`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html#addnode) caps how long a single node attempt may run.



Pass a number (milliseconds) or a [`TimeoutPolicy`](https://reference.langchain.com/javascript/langchain-langgraph/index/TimeoutPolicy) for separate run and idle limits:
...
You can set `run_timeout` / `runTimeout` and `idle_timeout` / `idleTimeout` together. Whichever fires first cancels the attempt.

#### Progress signals

Under the default `refresh_on="auto"` / `refreshOn: "auto"`, the idle clock resets on any of the following:
...
Set `refresh_on="heartbeat"` / `refreshOn: "heartbeat"` to narrow the refresh source to explicit `runtime.heartbeat()` calls only. This is useful when you want a strict idle definition that isn't reset by chatty subordinates:
...
<Note>
Requires a recent `@langchain/langgraph` release with node-level error handler support.
</Note>
...
Use `isNodeError(value)` to narrow unknown values in catch blocks. Handlers that don't need failure context can omit the `error` parameter and use a simpler `(state)` signature.
...
Instead of repeating the same `retryPolicy` or `cachePolicy` on every `addNode` call, use [`setNodeDefaults`](https://reference.langchain.com/javascript/langchain-langgraph/index/StateGraph#member-setNodeDefaults) to configure graph-wide defaults in one place:
...
Both `stepA` and `stepB` share the same retry and cache policies without duplication.

<Note>
In the JavaScript/TypeScript SDK, `setNodeDefaults` supports **`retryPolicy`** and **`cachePolicy`** only. Default `errorHandler` and `timeout` values are not yet supported—set those per node via `addNode` options.
</Note>
...
const graph = new StateGraph(State)
  ...
  .addNode("stepA", stepA) // uses default retry policy
  .addNode("stepB", stepB, { retryPolicy: { maxAttempts: 5 } }) // overrides default
  .addEdge(START, "stepA")
  .compile();
```
...
| Graph finishes naturally on the same tick as drain | Returns normally. Inspect `control.drain_requested` / `control.drainRequested` to distinguish from a normal run. |
...
- **`setNodeDefaults` is partial in JS**: supports `retryPolicy` and `cachePolicy` defaults only; set `errorHandler` and `timeout` per node via `addNode` options.
