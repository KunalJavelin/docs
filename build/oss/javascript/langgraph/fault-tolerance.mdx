...
Configure retries, timeouts, and error handlers per node via [`addNode`](https://reference.langchain.com/javascript/classes/_langchain_langgraph.index.StateGraph.html#addnode) options. `setNodeDefaults` is not yet available in the JavaScript/TypeScript SDK.
...
Pass a number (milliseconds) or a @[`TimeoutPolicy`] for separate run and idle limits:
