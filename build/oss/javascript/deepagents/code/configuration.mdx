...
Configure retry counts for transient model provider errors with the top-level `[retries]` section. Deep Agents Code passes these values through to provider integrations that accept `max_retries`. If you omit this section, the provider SDK default applies.
...
1. `--max-retries N`
2. `--model-params '{"max_retries": N}'`
3. `[models.providers.<provider>.params].max_retries`
