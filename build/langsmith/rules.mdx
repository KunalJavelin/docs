...
1. Select an action to trigger when the rule is applied. There are four actions you can take with an automation rule:
    ...
    - **Extend data retention**: Extends the data retention period on matching traces that use base retention [(refer to the data retention docs for more details)](/langsmith/observability#data-retention).
