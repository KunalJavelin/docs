...
<Note>
Event streaming requires `langgraph-api>=0.10.0` on the [LangGraph Agent Server](/langsmith/local-server). Managed LangSmith deployments are updated automatically; self-hosted servers must be on a compatible version. The client SDK must be `langgraph-sdk>=0.4.0` with `langchain-core>=1.4.0` (Python) or `@langchain/langgraph-sdk>=1.9.15` (JavaScript). Servers on earlier versions continue to serve the [legacy streaming API](/langsmith/streaming).
</Note>
...
| `thread.toolCalls` | Observe tool invocations with assembled input, streamed output, and result. |
...
- [Streaming API](/langsmith/streaming) — the legacy `stream_mode`-based streaming API. Still supported by `langgraph-api>=0.10.0` for existing applications.
