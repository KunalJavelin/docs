...
- **Deep Agents** — [LangSmith sandbox integration](/oss/python/integrations/sandboxes/langsmith): step-by-step guide to using `LangSmithSandbox` as a Deep Agents backend, including installation, backend creation, and cleanup.
- **Sandboxes as agent backends** — [Sandboxes overview](/oss/python/deepagents/sandboxes): configure any sandbox as the execution backend so your agent gets `execute` and filesystem tools automatically.
