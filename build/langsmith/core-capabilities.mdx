...
description: Overview of Agent Server core capabilities including streaming, human-in-the-loop, MCP, A2A, distributed tracing, webhooks, and double-texting.
---
...
<Card title="Event streaming API" icon="bolt" href="/langsmith/event-streaming">
  Stream messages, state, tool calls, and subgraphs from a deployed agent through typed projections. Recommended for `langgraph-api>=0.10.0rc1`.
</Card>

<Card title="Streaming API" icon="player-play" href="/langsmith/streaming">
  Legacy `stream_mode`-based API for streaming run outputs. Supported alongside event streaming for existing applications.
</Card>
