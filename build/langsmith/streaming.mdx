...
<Tip>
For new applications, the [event streaming API](/langsmith/event-streaming) is recommended—the typed-projection API for LangSmith deployments. Event streaming exposes separate iterators per projection (messages, values, tool calls, subgraphs, output) over one thread subscription, instead of branching on `stream_mode` chunks.
</Tip>
