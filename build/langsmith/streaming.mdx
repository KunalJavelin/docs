...
<Note>
The [event streaming API](/langsmith/event-streaming) is the typed-projection API for LangSmith deployments. It exposes separate iterators per projection (messages, values, tool calls, subgraphs, output) over one thread subscription, instead of branching on `stream_mode` chunks.
</Note>
