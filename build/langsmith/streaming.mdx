...
<Tip>
For new applications, we recommend the [event streaming API](/langsmith/event-streaming)—the typed-projection API for LangSmith deployments. Event streaming exposes separate iterators per projection (messages, values, tool calls, subgraphs, output) over one thread subscription, instead of branching on `stream_mode` chunks.
</Tip>
