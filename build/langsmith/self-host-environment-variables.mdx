...
By default, LangSmith logs a success message for every batch of runs it persists. On deployments that process a high volume of traces, this can produce excessive log noise.

To disable these success log messages, set `FF_PERSIST_BATCHED_RUNS_SUCCESS_LOGGING` to `false` on the `queue` and `ingestQueue` services:
...
<Note>
This flag defaults to `true`. Setting it to `false` suppresses per-batch success messages only — errors are still logged.
</Note>
